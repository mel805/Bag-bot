...
/**
 * Commandes Discord pour la gestion des sauvegardes
 */
...
class BackupCommands {
  ...
  getSlashCommands() {
    return [
      ...
      new SlashCommandBuilder()
        .setName('backup')
        .setDescription('Gestion des sauvegardes du bot')
        ...
        .addSubcommand(subcommand =>
          subcommand
            .setName('list')
            .setDescription('Afficher toutes les sauvegardes avec sélecteur')
        )
        .addSubcommand(subcommand =>
          subcommand
            .setName('status')
            .setDescription('Afficher le statut des sauvegardes')
        )
        .addSubcommand(subcommand =>
          subcommand
            .setName('restore')
  ...
  /**
   * Gestionnaire principal des commandes backup
   */
  async handleBackupCommand(interaction) {
    // Vérifier les permissions (admin seulement)
    if (!interaction.member.permissions.has('Administrator')) {
      return await interaction.reply({
        content: '❌ Seuls les administrateurs peuvent utiliser les commandes de sauvegarde.',
        ephemeral: true
      });
    }
    ...
    switch (subcommand) {
      case 'create':
        return await this.handleCreateBackup(interaction);
      case 'list':
        return await this.handleListBackups(interaction);
      case 'status':
        return await this.handleStatusBackup(interaction);
      case 'restore':
        return await this.handleRestoreBackup(interaction);
      default:
        return await interaction.reply({
  ...
  /**
   * Forcer la création d'une sauvegarde
   */
   ...
   */
  async handleStatusBackup(interaction) {
    await interaction.deferReply();

    try {
      const backups = this.backupSystem.getAllBackups();
      const configCount = this.freeboxSync.countFiles('/var/data/bot-backups/config/bot-config_*.json');
      const userDataCount = this.freeboxSync.countFiles('/var/data/bot-backups/userdata/user-data_*.json');
      ...
      const embed = new EmbedBuilder()
        .setTitle('📊 Statut des Sauvegardes')
        .setDescription('État actuel du système de sauvegarde')
        .addFields(
          { name: '🏠 Local', value: `${backups.length}/168 sauvegardes\n💾 ${Math.round(totalSize / 1024)} MB`, inline: true },
          { name: '☁️ Freebox', value: `Config: ${configCount}\nDonnées: ${userDataCount}`, inline: true },
          { name: '📅 Aujourd\'hui', value: `${todayBackups.length}/24 sauvegardes`, inline: true }
        )
        .setColor(0x0099FF)
        .setTimestamp();
      ...
      if (backups.length > 0) {
        ...
        embed.addFields(
          { name: '📅 Plus Récente', value: `${latest.timestamp}\n(${latest.age})`, inline: true },
          { name: '📅 Plus Ancienne', value: `${oldest.timestamp}\n(${oldest.age})`, inline: true },
          { name: '⚙️ Rétention', value: '7 jours (168h)', inline: true }
        );
      }
      ...
      console.error('❌ Erreur statut sauvegarde:', error);
      return await interaction.editReply({
        content: `❌ Erreur lors de la récupération du statut: ${error.message}`
      });
    }
  }

  /**
   * Lister toutes les sauvegardes avec pagination
   */
  ...
  async displayBackupPage(interaction, allBackups, page) {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`Page ${page + 1}/${maxPage + 1} • ${allBackups.length} sauvegardes total`)
      .setColor(0x0099FF)
      .setTimestamp();
  ...
  /**
   * Restaurer une sauvegarde par timestamp
   */
   ...
   */
  async handleBackupInteraction(interaction) {
    if (interaction.isButton()) {
      return await this.handleBackupButton(interaction);
    } else if (interaction.isStringSelectMenu()) {
      return await this.handleBackupSelect(interaction);
    }
  }
   ...
   */
  async handleBackupButton(interaction) {
   ...
   */
  async handleBackupSelect(interaction) {
