...
async function handleEconomyAction(interaction, actionKey) {
  ...
  const msgSet = {
    success: Array.isArray(cfgMsg.success) && cfgMsg.success.length ? cfgMsg.success : erotic.success,
    fail: Array.isArray(cfgMsg.fail) && cfgMsg.fail.length ? cfgMsg.fail : erotic.fail
  };
  let msgText = null;
  ...
  if (actionKey === 'tromper') {
    ...
    if (!third) {
      ...
      if (success) {
        const texts = [
          `Tu surprends {cible} avec un(e) autre, mais tu reprends le dessus.`,
          `Pris(e) en flagrant délit, tu transformes la scène en jeu torride.`,
          `Tu participes, renversant la situation en trio brûlant.`,
          `Découverte choquante, mais tu retournes l'infidélité en avantage.`,
          `Le piège devient orgie, {cible} succombe à ton pouvoir.`,
          `Tu domines la scène malgré la trahison.`,
          `Infidélité acceptée comme provocation torride.`,
          `Tu prends l'autre de court, {cible} reste à toi.`,
          `Scandale transformé en passion partagée.`,
          `Tu sors gagnant(e) de la trahison, {cible} comblé(e).`
        ];
        msgText = texts[randInt(0, texts.length - 1)];
      } else {
        const texts = [
          `Tu es pris(e) sur le fait, humiliation totale.`,
          `Trahison révélée, {cible} s'effondre.`,
          `Ton infidélité détruit la confiance, silence pesant.`,
          `Découvert(e) avec un(e) autre, {cible} fuit.`,
          `Tout s'écroule, jeu terminé.`
        ];
        msgText = texts[randInt(0, texts.length - 1)];
      }
  ...
  if (actionKey === 'massage') {
    const zones = ['dos','épaules','nuque','jambes','pieds','fesses'];
    const zoneOpt = String(interaction.options.getString('zone', false) || '').toLowerCase();
    const z = zones.includes(zoneOpt) ? zoneOpt : zones[randInt(0, zones.length - 1)];
    
    const massageMessages = {
      dos: {
        success: [
          'Tes mains glissent le long de son dos, détente et soupirs.',
          'Tes pouces pétrissent sa colonne, il/elle fond de plaisir.',
          'Chaque pression sur ses muscles dorsaux arrache un gémissement.',
          'Tes mains explorent son dos en caresses sensuelles.',
          'Tu alternes massage ferme et doux, son dos frémit.'
        ],
        fail: ['Tes gestes sont trop brusques, {cible} grimace.']
      },
      épaules: {
        success: [
          'Tes doigts malaxent ses trapèzes, soupir de soulagement.',
          'Tu détends ses épaules raides, il/elle ferme les yeux.',
          'Chaque pression arrache un grognement de plaisir.',
          'Tu fais craquer doucement ses épaules, soupir profond.',
          'Ton massage glisse de l\'épaule au cou, frisson garanti.'
        ],
        fail: ['Massage maladroit, détente ratée.']
      },
      nuque: {
        success: [
          'Tes pouces pressent sa nuque, excitation subtile.',
          'Tu masses sa nuque lentement, il/elle penche la tête.',
          'Ton souffle brûlant accompagne ton massage.',
          'Chaque pression rend sa nuque plus sensible encore.',
          'Tu alternes caresses et pressions, frisson électrique.'
        ],
        fail: ['Tu appuies au mauvais endroit, il/elle proteste.']
      },
      jambes: {
        success: [
          'Tes mains glissent sur ses cuisses, frisson immédiat.',
          'Tu masses ses mollets fermes, soupir d\'aise.',
          'Tu remontes lentement vers ses cuisses intérieures, tension montée.',
          'Tes mains s\'attardent sur ses jambes, caresse sensuelle.',
          'Tu presses ses cuisses, son souffle s\'accélère.'
        ],
        fail: ['Tes mains s\'égarent, {cible} les repousse.']
      },
      pieds: {
        success: [
          'Tu masses doucement ses voûtes plantaires, il/elle rit nerveusement.',
          'Tes pouces travaillent ses talons, détente totale.',
          'Chaque pression sur ses pieds lui arrache un soupir.',
          'Tu embrasses ses orteils entre deux pressions.',
          'Tu alternes massage et caresses taquines sur ses pieds.'
        ],
        fail: ['Ambiance cassée, massage stoppé.']
      },
      fesses: {
        success: [
          'Tes mains pétrissent lentement ses fesses, soupir coquin.',
          'Tu alternes pression ferme et caresses sensuelles.',
          'Chaque massage sur ses fesses le/la fait se cambrer.',
          'Tu caresses et masses ses fesses, désir montant.',
          'Tes pouces pressent ses reins puis glissent sur ses fesses.'
        ],
        fail: ['Tes mains s\'égarent, {cible} les repousse.']
      }
    };
    
    const zoneMessages = massageMessages[z];
    if (zoneMessages) {
      const texts = success ? zoneMessages.success : zoneMessages.fail;
      msgText = texts[randInt(0, texts.length - 1)];
    }
  ...
  if (actionKey === 'caress') {
    ...
    const zoneMessages = caressMessages[z];
    if (zoneMessages) {
      const texts = success ? zoneMessages.success : zoneMessages.fail;
      msgText = texts[randInt(0, texts.length - 1)];
    }
  ...
  if (actionKey === 'pillowfight') {
    const pillowfightMessages = {
      success: [
        ...
        'La bataille se transforme en étreinte torride.',
        'Plumes dans les cheveux, vos lèvres se retrouvent.',
        'Un coup d\'oreiller suivi d\'un baiser fougueux.',
        'Vos rires deviennent gémissements.',
        'Oreillers éclatés, lit transformé en champ de bataille sexy.'
      ],
      fail: [
        'Oreillers introuvables, bataille annulée.',
        'Tu frappes trop fort, {cible} râle.',
        'Oreiller éventré trop tôt, ambiance cassée.',
        'Pas d\'humeur pour jouer ce soir.',
        'Un coup mal placé, plainte immédiate.'
      ]
    };
    
    const texts = success ? pillowfightMessages.success : pillowfightMessages.fail;
    msgText = texts[randInt(0, texts.length - 1)];
  }
  if (actionKey === 'oops') {
    if (success) {
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'restore') {
      try {
    return showRestoreMenu(interaction);
        const ok = await isStaffMember(interaction.guild, interaction.member);
        if (!ok) return interaction.reply({ content: '⛔ Réservé au staff.', ephemeral: true });
