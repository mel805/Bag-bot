...
async function handleEconomyAction(interaction, actionKey) {
  ...
  // Nettoyer les variables globales au début pour éviter les pings parasites
  ...
  // Récupérer la cible (optionnelle pour les actions action_)
  ...
  const eroticMsgBank = {
    ...
    // Caress par zone (cible obligatoire maintenant)
    caress_sein: {
      success: [
    ...
    caress_corps: {
      success: [
        'Tes mains explorent tout son corps, {cible} frissonne.',
        'Tu caresses chaque courbe, {cible} se cambre.',
        'Tes doigts glissent sur sa peau, chaleur montante.',
        'Tu explores son corps entier, {cible} soupire.',
    ...
    lick: {
      success: [
        'Tu lèches {cible} avec gourmandise.',
        'Coups de langue sensuels sur {cible}.',
        'Tu explores {cible} avec ta langue.',
        'Léchage passionné de {cible}.',
        'Saveur délicieuse de {cible}.'
      ],
      fail: [
        'Léchage refusé par {cible}.',
        '{cible} n\'est pas d\'humeur.',
        'Tu es repoussé gentiment par {cible}.'
      ]
    },
    lick_seins: {
      success: [
    ...
    lick_cul: {
      ...
      fail: ['Ton audace est stoppée net, {cible} secoue la tête.']
    },
    lick_oreille: {
      success: [
    ...
    // Action suck avec zones
    suck: {
      success: [
        'Tu suces {cible} avec passion.',
        'Aspiration sensuelle de {cible}.',
        'Succion parfaite sur {cible}.',
        'Plaisir intense avec {cible}.',
        'Technique maîtrisée sur {cible}.'
      ],
      fail: [
        'Succion refusée par {cible}.',
        'Pas le bon moment.',
        'Geste repoussé par {cible}.'
      ]
    },
    suck_bite: {
      success: [
    ...
    suck_téton: {
      success: [
        'Tu suces son téton avec avidité, {cible} gémit.',
    ...
    // Action nibble avec zones
    nibble: {
      success: [
        'Tu mordilles {cible} avec douceur.',
        'Petites morsures taquines sur {cible}.',
        'Mordillements sensuels de {cible}.',
        'Tes dents effleurent {cible}.',
        'Jeu de morsures avec {cible}.'
      ],
      fail: [
        'Morsure refusée par {cible}.',
        '{cible} est trop sensible.',
    ...
    nibble_lèvres: {
      success: [
        ...
        'Ta morsure douce l\'excite, {cible} gémit.',
      ...
      fail: ['{cible} esquive, pas de morsure aujourd\'hui.']
    },
    nibble_épaule: {
      success: [
    ...
    pillowfight: {
      success: [
        'Tu frappes {cible} d\'un oreiller, éclat de rire.',
        'Pluie de plumes, vos corps s\'entrechoquent.',
        'Le combat dégénère en roulade passionnée.',
        'Oreillers volent, baisers volés entre deux coups.',
        'Tu plaques {cible} avec ton oreiller, souffle court.'
      ],
      fail: [
        'Oreillers introuvables, bataille annulée.',
        'Tu frappes trop fort, {cible} râle.',
        'Oreiller éventré trop tôt, ambiance cassée.'
      ]
    }
  };
  ...
  if (!msgText) {
    // Vérifier si une zone est spécifiée pour les actions qui en supportent
    const zoneOpt = String(interaction.options?.getString?.('zone', false) || '').toLowerCase();
    let actionWithZone = actionKey;
    ...
    if (actionKey === 'lick' && zoneOpt) {
      const validZones = ['seins', 'chatte', 'cul', 'oreille', 'ventre', 'bite'];
      if (validZones.includes(zoneOpt)) {
        actionWithZone = `lick_${zoneOpt}`;
      }
    }
    
    // Pour suck, utiliser la version avec zone si spécifiée
    if (actionKey === 'suck' && zoneOpt) {
      const validZones = ['bite', 'téton', 'oreille'];
      if (validZones.includes(zoneOpt)) {
        actionWithZone = `suck_${zoneOpt}`;
      }
    }
    
    // Pour nibble, utiliser la version avec zone si spécifiée
    if (actionKey === 'nibble' && zoneOpt) {
      const validZones = ['cou', 'lèvres', 'épaule', 'lobe'];
      if (validZones.includes(zoneOpt)) {
        actionWithZone = `nibble_${zoneOpt}`;
      }
    }
    
    // Pour doigter, utiliser la version avec zone si spécifiée  
    if (actionKey === 'doigter' && zoneOpt) {
      const validZones = ['chatte', 'cul'];
      if (validZones.includes(zoneOpt)) {
        actionWithZone = `doigter_${zoneOpt}`;
      }
    }
    
    // Pour fuck, utiliser la version avec zone si spécifiée
    if (actionKey === 'fuck' && zoneOpt) {
      const validZones = ['chatte', 'bite'];
      if (validZones.includes(zoneOpt)) {
        actionWithZone = `fuck_${zoneOpt}`;
      }
    }
    
    // Pour orgasme, utiliser la version avec zone si spécifiée
    if (actionKey === 'orgasme' && zoneOpt) {
      const validZones = ['chatte', 'bite', 'corps'];
      if (validZones.includes(zoneOpt)) {
        actionWithZone = `orgasme_${zoneOpt}`;
      }
    }
    
    
    // Utiliser eroticMsgBank en priorité si disponible
    const erotic = eroticMsgBank[actionWithZone] || eroticMsgBank[actionKey] || { success: [], fail: [] };
    const finalMsgSet = {
      success: Array.isArray(erotic.success) && erotic.success.length ? erotic.success : msgSet.success,
      fail: Array.isArray(erotic.fail) && erotic.fail.length ? erotic.fail : msgSet.fail
    };
    
    msgText = success
      ? (Array.isArray(finalMsgSet.success) && finalMsgSet.success.length ? finalMsgSet.success[Math.floor(Math.random()*finalMsgSet.success.length)] : null)
      : (Array.isArray(finalMsgSet.fail) && finalMsgSet.fail.length ? finalMsgSet.fail[Math.floor(Math.random()*finalMsgSet.fail.length)] : null);
  }
  
  // Remplacer {cible} dans les messages
  if (msgText && msgText.includes('{cible}')) {
    if (initialPartner) {
      // Avec cible : utiliser la mention
      msgText = msgText.replace(/\{cible\}/gi, String(initialPartner));
    } else {
      // Sans cible : utiliser des messages adaptés pour action solo
      if (actionKey === 'caress') {
        msgText = success ? 'Tu te caresses avec douceur.' : 'Tu n\'es pas d\'humeur à te caresser.';
      } else if (actionKey === 'lick') {
        msgText = success ? 'Tu te lèches les lèvres sensuellement.' : 'Tu n\'as pas envie de lécher.';
      } else if (actionKey === 'suck') {
        msgText = success ? 'Tu suces ton doigt de manière suggestive.' : 'Tu n\'es pas d\'humeur.';
      } else if (actionKey === 'nibble') {
        msgText = success ? 'Tu te mordilles les lèvres.' : 'Tu n\'as pas envie de mordiller.';
      } else if (actionKey === 'kiss') {
        msgText = success ? 'Tu t\'embrasses dans le miroir.' : 'Tu n\'arrives pas à t\'embrasser.';
      } else if (actionKey === 'flirt') {
        msgText = success ? 'Tu pratiques ton charme devant le miroir.' : 'Ton charme ne fonctionne pas aujourd\'hui.';
      } else if (actionKey === 'seduce') {
        msgText = success ? 'Tu travailles ta séduction.' : 'Tu n\'es pas en forme pour séduire.';
      } else if (actionKey === 'dance') {
        msgText = success ? 'Tu danses avec style et sensualité.' : 'Tu n\'es pas d\'humeur à danser.';
      } else if (actionKey === 'massage') {
        msgText = success ? 'Tu te masses pour te détendre.' : 'Tu n\'es pas d\'humeur pour un massage.';
      } else if (actionKey === 'hairpull') {
        msgText = success ? 'Tu tires tes propres cheveux, geste nerveux.' : 'Tu n\'as pas envie de te tirer les cheveux.';
      } else if (actionKey === 'shower') {
        msgText = success ? 'Tu prends une douche relaxante.' : 'Tu n\'as pas envie de prendre une douche.';
      } else if (actionKey === 'bed') {
        msgText = success ? 'Tu t\'allonges sur le lit.' : 'Tu n\'as pas envie de te coucher.';
      } else if (actionKey === 'pillowfight') {
        msgText = success ? 'Tu fais une bataille d\'oreillers solo.' : 'Pas d\'humeur pour jouer.';
      } else {
        // Pour les autres actions, supprimer {cible}
        msgText = msgText.replace(/\{cible\}/gi, '').replace(/,\s*$/, '').replace(/\s+/g, ' ').trim();
      }
    }
  }
  ...
  if (success) {
    try {
      let partnerUser = null;
      // Récupérer la cible selon le type d'action
      if (actionKey === 'tromper') {
        partnerUser = tromperResolvedPartner || interaction.options.getUser('cible', false);
      } else if (actionKey === 'crime') {
        partnerUser = interaction.options.getUser('complice', false);
      } else {
        partnerUser = interaction.options.getUser('cible', false);
      }
      // Créer partnerField (cible maintenant toujours obligatoire)
  ...
  // Ne pas ajouter automatiquement la cible au contenu
  const parts = [];
  if (imageLinkForContent) parts.push(imageLinkForContent);
  
  // Add pings for special actions
  // Ajouter les pings seulement pour tromper et orgie
  if (actionKey === 'orgie' && global.__eco_orgie_pings) {
    parts.push(`🔥 ${global.__eco_orgie_pings}`);
  }
  if (actionKey === 'tromper' && global.__eco_tromper_pings) {
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isChatInputCommand() && (interaction.commandName === 'caresser' || interaction.commandName === 'action_caresser')) {
    ...
    if (interaction.isChatInputCommand() && (interaction.commandName === 'chatouiller' || interaction.commandName === 'action_chatouiller')) {
      return handleEconomyAction(interaction, 'tickle');
    }
    // Gestionnaires avec préfixe action_ pour toutes les actions
    ...
    if (interaction.isChatInputCommand() && (interaction.commandName === 'seduire' || interaction.commandName === 'action_seduire')) {
      return handleEconomyAction(interaction, 'seduce');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'branler' || interaction.commandName === 'action_branler')) {
      return handleEconomyAction(interaction, 'branler');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'doigter' || interaction.commandName === 'action_doigter')) {
      return handleEconomyAction(interaction, 'doigter');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'fuck' || interaction.commandName === 'action_fuck')) {
      return handleEconomyAction(interaction, 'fuck');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'sodo' || interaction.commandName === 'action_sodo')) {
      return handleEconomyAction(interaction, 'sodo');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'orgasme' || interaction.commandName === 'action_orgasme')) {
      return handleEconomyAction(interaction, 'orgasme');
    }
    // Actions lifestyle avec préfixe action_
    if (interaction.isChatInputCommand() && (interaction.commandName === 'tirercheveux' || interaction.commandName === 'action_hairpull' || interaction.commandName === 'action_tirercheveux')) {
      return handleEconomyAction(interaction, 'hairpull');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'shower' || interaction.commandName === 'action_shower')) {
      return handleEconomyAction(interaction, 'shower');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'bed' || interaction.commandName === 'action_bed')) {
      return handleEconomyAction(interaction, 'bed');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'collar' || interaction.commandName === 'action_collar')) {
      return handleEconomyAction(interaction, 'collar');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'leash' || interaction.commandName === 'action_leash')) {
      return handleEconomyAction(interaction, 'leash');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'kneel' || interaction.commandName === 'action_kneel')) {
      return handleEconomyAction(interaction, 'kneel');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'order' || interaction.commandName === 'action_order')) {
      return handleEconomyAction(interaction, 'order');
    }
    if (interaction.isChatInputCommand() && (interaction.commandName === 'pillowfight' || interaction.commandName === 'action_pillowfight')) {
