...
async function handleEconomyAction(interaction, actionKey) {
  ...
  // Seules tromper et orgie ont besoin de sélection automatique de cible
  const actionsWithAutoTarget = ['tromper','orgie'];
  let initialPartner = null;
  let tromperResolvedPartner = null;
  try {
    // Pour toutes les actions, récupérer la cible seulement si fournie
    initialPartner = interaction.options.getUser('cible', false);
    
    // Sélection automatique seulement pour tromper et orgie
    if (actionsWithAutoTarget.includes(actionKey) && !initialPartner) {
      // Logique de sélection automatique pour tromper/orgie uniquement
      try {
        let pick = null;
  ...
  if (!msgText) {
    ...
    // Debug pour voir l'action finale utilisée
    if (actionKey === 'caress') {
      console.log(`[DEBUG ACTION] ActionWithZone: "${actionWithZone}" - Original: "${actionKey}"`);
    }
  ...
  if (success) {
    try {
      ...
      // Créer partnerField seulement si l'utilisateur a explicitement choisi une cible
      const userChoseTarget = interaction.options.getUser('cible', false) !== null;
      if (partnerUser && !partnerUser.bot && partnerUser.id !== interaction.user.id && userChoseTarget) {
