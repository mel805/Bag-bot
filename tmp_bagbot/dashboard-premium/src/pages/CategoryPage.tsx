          ...
          <div className="space-y-3">
            ...
            <div className="grid md:grid-cols-2 gap-3">
              <label className="text-white/70">Image en bas (URL)
                ...
                <input type="file" accept="image/*" className="mt-2 text-white/70" onChange={async e=>{ const f=e.target.files?.[0]; if (!f) return; const fr=new FileReader(); fr.onloadend=async()=>{ const dataUrl=String(fr.result||''); if (!dataUrl.startsWith('data:')) { alert('Fichier invalide.'); return; } const url=await uploadBase64(f.name, dataUrl); if (url) setTicketsBannerUrl(url); else alert('Échec de l\'upload de l\'image.'); }; fr.readAsDataURL(f); }} />
              </label>
