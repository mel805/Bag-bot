...
async function sendDetailedBackupLog(guild, info, method, user) {
  try {
    ...
    const hasChannel = Boolean((lc?.channels && lc.channels.backup) || lc?.channelId);
    if (!hasChannel) return;

    const timestamp = new Date(info.details?.timestamp || new Date()).toLocaleString('fr-FR');
    
    // Déterminer le statut global (sans GitHub)
    const localSuccess = !!info.local?.success;
    const freeboxSuccess = !!info.freebox?.success;
    const anySuccess = localSuccess || freeboxSuccess;
    const globalStatus = anySuccess ? '✅ Succès' : '❌ Échec';
    const statusColor = anySuccess ? 0x44ff44 : 0xff4444;
    ...
    if (localSuccess) {
      ...
      if (info.backupFile) {
        const name = String(info.backupFile).split('/').pop();
        localValue += `\n📄 Fichier: ${name}`;
      }
    ...
    // Statut sauvegarde Freebox
    const fb = info.freebox || {};
    let fbValue = (fb.available === false) ? '⚠️ Freebox non disponible' : (fb.success ? '✅ Copie enregistrée' : (fb.error ? `❌ ${fb.error}` : '—'));
    if (fb.success && fb.filename) {
      const short = String(fb.filename).split('/').pop();
      fbValue += `\n📄 Fichier: ${short}`;
    }
    embed.fields.push({ name: '🗄️ Freebox', value: fbValue, inline: true });
            ...
            try {
              ...
              try {
                ...
                if (ch && ch.isTextBased?.()) {
                  ...
                  const opts = cats.slice(0, 25).map((c) => {
                    const label = String(c.label || c.key || 'Catégorie').slice(0, 100) || 'Catégorie';
                    const value = String(c.key || label).slice(0, 100);
                    const desc = String(c.description || '').slice(0, 100);
                    const opt = { label, value };
                    if (desc) opt.description = desc;
                    if (c.emoji) opt.emoji = { name: String(c.emoji) };
                    return opt;
                  });
                  ...
                  const sent = await ch.send({ content: null, embeds: [embed], components: [row], files: __banner ? [__banner] : [] }).catch(()=>null);
                  if (sent && sent.id) {
...
async function handleEconomyAction(interaction, actionKey) {
  ...
  const content = parts.filter(Boolean).join('\n') || undefined;
  // Build allowedMentions to ensure pings are delivered (edits do not ping)
  const _mentionUserIds = [];
  try { if (initialPartner && initialPartner.id) _mentionUserIds.push(String(initialPartner.id)); } catch (_) {}
  try {
    if (global.__eco_orgie_pings) {
      const ids = String(global.__eco_orgie_pings).match(/<@(\d+)>/g) || [];
      for (const tag of ids) { const id = tag.replace(/[^0-9]/g, ''); if (id) _mentionUserIds.push(id); }
    }
  } catch (_) {}
  try {
    if (global.__eco_tromper_pings) {
      const ids = String(global.__eco_tromper_pings).match(/<@(\d+)>/g) || [];
      for (const tag of ids) { const id = tag.replace(/[^0-9]/g, ''); if (id) _mentionUserIds.push(id); }
    }
  } catch (_) {}
  ...
  try {
    ...
    return await respondAndUntrack({ content, embeds: [embed], files: imageAttachment ? [imageAttachment.attachment] : undefined, ...(allowedMentions ? { allowedMentions } : {}) }, preferFollowUp);
  } catch (error) {
...
async function buildConfigEmbed(guild) {
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      // N'afficher que les rôles Staff configurés (ne pas montrer un placeholder global)
      ...(staffIds.length ? [{ name: 'Rôles Staff', value: staffList }] : []),
...
async function buildStaffRemoveRows(guild) {
  const staffIds = await getGuildStaffRoleIds(guild.id).catch(() => []);
  const staffRoles = Array.isArray(staffIds)
    ? staffIds.map((id) => guild.roles.cache.get(id)).filter(Boolean)
    : [];

  const select = new StringSelectMenuBuilder()
    .setCustomId('staff_remove_roles')
    .setPlaceholder("Sélectionner les rôles Staff à RETIRER…");

  if (staffRoles.length) {
    const options = staffRoles.slice(0, 25).map((r) => ({ label: r.name, value: r.id }));
    select.addOptions(options).setMinValues(1).setMaxValues(Math.min(staffRoles.length, 25));
  } else {
    select.addOptions({ label: 'Aucun rôle Staff configuré', value: 'none' }).setDisabled(true);
  }
...
client.once(Events.ClientReady, async (readyClient) => {
      ...
      for (const [channelId, rec] of entries) {
        ...
        try {
          ...
          if (transcriptChannel && transcriptChannel.isTextBased?.()) {
            ...
            const chNameSafe = ch?.name ? String(ch.name) : ('#'+ch.id);
            const htmlDoc = `<!doctype html><html><head><meta charset="utf-8"><title>Transcription ${esc(chNameSafe)}</title><style>body{font-family:system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Ubuntu,\"Helvetica Neue\",Arial,sans-serif;background:#0b0f12;color:#e0e6ed;margin:16px} h2{margin:0 0 8px 0} .meta{color:#90a4ae;white-space:pre-wrap;margin-bottom:8px} .time{color:#90a4ae} .msg{margin:4px 0} .content{white-space:pre-wrap} .content.member{color:#4caf50} .content.staff{color:#ffb74d} .content.bot{color:#64b5f6}</style></head><body><h2>Transcription du ticket ${esc(chNameSafe)}</h2><div class=\"meta\">${esc(head)}</div><hr/>${htmlLines.join('\\n')}</body></html>`;
  ...
  // Emojis
  client.on(Events.GuildEmojiCreate, async (emoji) => { try { const cfg = await getLogsConfig(emoji.guild.id); if (!cfg.categories?.emojis) return; const acts=(cfg.actions?.emojis||[]); if (acts.length && !acts.includes('create')) return; const e=new EmbedBuilder().setColor(0x55ff55).setTitle(`${(cfg.emojis&&cfg.emojis['emojis'])||cfg.emoji} Emoji créé`).setDescription(`${emoji.name}`); await sendLog(emoji.guild,'emojis',e);}catch(_){} });
  client.on(Events.GuildEmojiDelete, async (emoji) => { try { const cfg = await getLogsConfig(emoji.guild.id); if (!cfg.categories?.emojis) return; const acts=(cfg.actions?.emojis||[]); if (acts.length && !acts.includes('delete')) return; const e=new EmbedBuilder().setColor(0xff5555).setTitle(`${(cfg.emojis&&cfg.emojis['emojis'])||cfg.emoji} Emoji supprimé`).setDescription(`${emoji.name}`); await sendLog(emoji.guild,'emojis',e);}catch(_){} });
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    // Pagination des sauvegardes (Précédent/Suivant)
    if (interaction.isButton() && (interaction.customId.startsWith('restore_page_prev:') || interaction.customId.startsWith('restore_page_next:'))) {
      const parts = interaction.customId.split(':')[1];
      let current = 0, total = 1;
      if (parts && parts.includes('/')) {
        const [idx, tot] = parts.split('/');
        current = Math.max(0, Number(idx) || 0);
        total = Math.max(1, Number(tot) || 1);
      } else {
        current = Math.max(0, Number(parts) || 0);
      }
      try {
        const { listFreeboxBackups } = require('./storage/jsonStore');
        const all = await listFreeboxBackups();
        if (!all || all.length === 0) return interaction.update({ content: '❌ Aucun fichier de sauvegarde trouvé.', components: [] });
        const pages = [];
        for (let i = 0; i < all.length; i += 25) pages.push(all.slice(i, i + 25));
        total = pages.length;
        if (interaction.customId.startsWith('restore_page_prev:')) current = Math.max(0, current - 1);
        if (interaction.customId.startsWith('restore_page_next:')) current = Math.min(total - 1, current);
        ...
        const prevBtn = new ButtonBuilder().setCustomId(`restore_page_prev:${current}`).setLabel('Précédent').setStyle(ButtonStyle.Secondary).setDisabled(current === 0);
        const nextBtn = new ButtonBuilder().setCustomId(`restore_page_next:${current + 1}/${total}`).setLabel('Suivant').setStyle(ButtonStyle.Primary).setDisabled(current >= total - 1);
        rows.push(new ActionRowBuilder().addComponents(prevBtn, nextBtn));
        return interaction.update({ content: `🗃️ Choisissez un fichier à restaurer : (${current + 1}/${total})`, components: rows });
      } catch (e) {
        return interaction.update({ content: '❌ Erreur pagination: ' + (e?.message || e), components: [] });
      }
    }
    ...
    if (interaction.isStringSelectMenu() && interaction.customId === 'ticket_open') {
      ...
      // Mention the user and optionally ping staff roles (filter deleted roles)
      ...
      if (t.pingStaffOnOpen) {
        try {
          ...
          const validRoles = Array.isArray(pings) ? pings.map((id) => interaction.guild.roles.cache.get(id)).filter(Boolean) : [];
          if (validRoles.length) {
            const mentions = validRoles.map((r) => `<@&${r.id}>`).join(' ');
            content += `\n${mentions}`;
            allowedRoleIds = validRoles.map((r) => r.id);
          }
        } catch (_) {}
      }
    ...
    if (interaction.isButton() && interaction.customId === 'ticket_close') {
      ...
      try {
        ...
        async function buildTranscriptPayload() {
          ...
          const chNameSafe = interaction.channel?.name ? String(interaction.channel.name) : ('#'+interaction.channel.id);
          const htmlDoc = `<!doctype html><html><head><meta charset=\"utf-8\"><title>Transcription ${esc(chNameSafe)}</title><style>body{font-family:system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Ubuntu,\"Helvetica Neue\",Arial,sans-serif;background:#0b0f12;color:#e0e6ed;margin:16px} h2{margin:0 0 8px 0} .meta{color:#90a4ae;white-space:pre-wrap;margin-bottom:8px} .time{color:#90a4ae} .msg{margin:4px 0} .content{white-space:pre-wrap} .content.member{color:#4caf50} .content.staff{color:#ffb74d} .content.bot{color:#64b5f6}</style></head><body><h2>Transcription du ticket ${esc(chNameSafe)}</h2><div class=\"meta\">${esc(head)}</div><hr/>${htmlLines.join('\\n')}</body></html>`;
    ...
    if (interaction.isAnySelectMenu && interaction.customId === 'staff_remove_roles') {
      if (interaction.isStringSelectMenu() && interaction.values.includes('none')) {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'backup') {
      try {
        ...
        const createdName = (info && (info.backupFile || info.files?.backupFile)) ? String(info.backupFile || info.files.backupFile).split('/').pop() : null;
        const file = { attachment: json, name: createdName || 'bag-backup.json' };
        try {
          await sendDetailedBackupLog(interaction.guild, info, 'slash', interaction.user);
        } catch (_) {}
        const line = createdName ? `📦 Sauvegarde créée: ${createdName}` : '📦 Sauvegarde générée.';
        return interaction.editReply({ content: line, files: [file] });
      } catch (e) {
        try {
    ...
    if (interaction.isChatInputCommand() && interaction.commandName === 'restore') {
      try {
        ...
        // Pagination par lots de 25
        const pages = [];
        for (let i = 0; i < backups.length; i += 25) pages.push(backups.slice(i, i + 25));
        const page0 = pages[0];
        ...
        const rows = [new ActionRowBuilder().addComponents(selectMenu)];
        if (pages.length > 1) {
          const prevBtn = new ButtonBuilder().setCustomId('restore_page_prev:0').setLabel('Précédent').setStyle(ButtonStyle.Secondary).setDisabled(true);
          const nextBtn = new ButtonBuilder().setCustomId(`restore_page_next:1/${pages.length}`).setLabel('Suivant').setStyle(ButtonStyle.Primary);
          rows.push(new ActionRowBuilder().addComponents(prevBtn, nextBtn));
        }

        await interaction.reply({ content: `🗃️ Choisissez un fichier à restaurer : (1/${pages.length})`, components: rows, ephemeral: true });
      } catch (e) {
        try {
          const errorResult = {
            ok: false,
